...
public class TimetableRepositoryImpl extends BaseRespository implements TimetableRepository {
    ...
    public boolean saveTimetable(Timetable timetable) {
        if(0 == timetable.getId()){
            ...
            String sql ="insert into timetable(`id`,`name`,`time`,`freelancerId`,`content`) values(?,?,?,?,?)";
            if(-1 != update(sql, Integer.parseInt(id),timetable.getName(),timetable.getTime(),
                    timetable.getFreelancerId(),timetable.getContent()))
                return true;
            else return false;
        }else{
            String sql = "update timetable set `name`=?,`time`=?,`content`=? where id = ?";
            if(-1 != update(sql,timetable.getName(),timetable.getTime(),
                    timetable.getContent(),timetable.getId())){
                return true;
            }else return false;
        }
    }
